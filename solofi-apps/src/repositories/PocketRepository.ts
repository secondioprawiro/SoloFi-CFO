...
export class PocketRepository {
  ...
  async saveRules(userId: string, rules: PocketRuleInput[]): Promise<Pocket[]> {
    ...
    assertNoSupabaseError(deleteError, 'PocketRepository.saveRules (delete)');
    ...
    assertNoSupabaseError(insertError, 'PocketRepository.saveRules (insert)');
  ...
  async getRules(userId: string): Promise<Pocket[]> {
    ...
    assertNoSupabaseError(error, 'PocketRepository.getRules');
    return (data as Pocket[]) ?? [];
  }
}
